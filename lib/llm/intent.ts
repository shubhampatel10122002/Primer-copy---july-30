...
export async function classifyIntent(args: {
  ...
  storyPremise: string;
  /** Whether they tapped the owl or just started talking while reading. */
  source?: 'button' | 'off_script';
}): Promise<IntentResult> {
  const { transcript, currentPassage, currentWord, storyPremise, source = 'button' } = args;

  if (!transcript || transcript.trim().length < 2) {
    return { intent: 'unclear', interestTopic: null, fact: null, factKind: null, reasoning: 'empty transcript' };
  }
  ...
  try {
    ...
    return { intent: 'unclear', interestTopic: null, fact: null, factKind: null, reasoning: 'classifier error' };
  }
}
